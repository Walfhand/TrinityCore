...
The normal flow needs two players to start a 1v1 match, which is painful when
testing alone. A dev-only helper launches a Nexus match with a single player.

Enable it in `docker/configs/worldserver.conf` (generated by `make config`):

```ini
Moba.DevSoloMode = 1
```

Restart the worldserver to apply (no rebuild needed once the option exists):

```bash
docker compose -f docker/docker-compose.yml up -d --force-recreate worldserver
```

When enabled, the lobby NPC shows an extra option `DEV: Match solo
(instantane)`. Selecting it creates the arena instance, sends the native BG
invite popup, and starts the match as soon as you enter, without waiting for an
opponent. The enemy Nexus is still spawned so the victory loop can be tested.
Keep `Moba.DevSoloMode = 0` on any real server.
